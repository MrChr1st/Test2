...
async def _finalize_request(message, state, db, config, lang, method, submethod, bot):
    ...
    if method == "card" and submethod == "card_number":
        ...
        await message.answer(
            ("Действия ниже:" if lang == "ru" else "Actions below:"),
            reply_markup=None,
        )
        await message.answer(
            ("Используйте кнопки под сообщением." if lang == "ru" else "Use the buttons under the message."),
            reply_markup=None,
        )
        await message.answer(
            (" " if lang == "ru" else " "),
        ...
        await message.answer(
            ("Нажмите кнопку ниже." if lang == "ru" else "Press the button below."),
            reply_markup=None,
        )
        await message.answer(
            (" " if lang == "ru" else " "),
...
async def exchange_crypto_method(message: Message, db, config, state: FSMContext, bot):
    ...
    if not asset:
        await message.answer(TEXTS[lang]["crypto_invalid_choice"], reply_markup=crypto_choice_kb(lang))
        return
